...
| - | - |
| `relation_id` | 关联目标项 |
| `relation_type` | 目标项的类型，`1` 任务 `2` 页面 `3` 文件 |
| `name` | 名称 |
...
| - | - |
| `entity_id` | 实体ID |
| `type`      | `1` 页面组 `2` 页面 `3` 项目 `4` 任务 `5` 用户 |
| `name`      | 标题 |
| `create_at` | 创建时间 |
...
参数二选一，指定`user_id`时返回某用户的所有权限指定项，指定`entity_id`时返回某实体的所有权限指定项
...
| 名称 | 备注 |
| - | - |
| `entity_id` | 实体ID，仅当参数指定为`user_id`时有效 |
| `user_id`   | 用户ID，仅当参数指定为`entity_id_id`时有效 |
| `access`    | 权限掩码 |

## POST `/api/modify_acl`

更改权限控制。仅管理员和实体所有者可修改。
...
| `entity_id` | 实体ID |
| `access` | 权限掩码 |
| `is_remove` | `true` = 移除`access`中指定的位，`false` = 添加`access`中指定的位 |

## POST `/api/modify_acl_user`
...
| `user_id` | 用户ID |
| `entity_id` | 实体ID |
| `is_remove` | `true` = 从指定实体的ACL中移除用户，`false` = 移除用户 |
